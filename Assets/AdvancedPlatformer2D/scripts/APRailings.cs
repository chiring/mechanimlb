...
public class APRailings : MonoBehaviour 
{
	...
	public class Settings
	{
		...
		public string m_animClimbUp = "ClimbUp";				// anim name for climb up
		public string m_animClimbRight = "ClimbRight";			// anim name for climb right
		public string m_animClimbDown = "ClimbDown";			// anim name for climb down
		public string m_animClimbLeft = "ClimbLeft";			// anim name for climb left
	}
	...
	void Awake()
	{
		...
		if(colliders.Length > 0)
		{
			...
			for(int i = 0; i < colliders.Length; i++)
			{
				...
				Vector2 pointA = transform.TransformPoint(collider.center - 0.5f * collider.size);
				Vector2 pointB = transform.TransformPoint(collider.center + 0.5f * collider.size);
	...
	void LateUpdate()
	{
		...
		if(m_isControlling)
		{
			...
			// set speed
			float fSpeed = Mathf.Max(fAbsY, fAbsX) / oSettings.m_maxSpeed;
			m_player.GetAnim().speed = fSpeed;

			// set animation
			MoveDir ePrevMoveDir = m_moveDir;
			string sAnim = oSettings.m_animClimbUp;
			if(fSpeed < 1e-3f)
			{
			...
			{
				m_player.GetAnim().Play(sAnim, 0, 0f);
			}
		}
	}
	...
	void UpdateRailings () 
	{
		...
		if(m_isControlling)
		{
			...
			// update velocity
			m_motor.m_velocity.x = APInput.Update(m_motor.m_velocity.x, oSettings.m_maxSpeed * m_player.m_inputs.m_axisX.GetRawValue(), true, oSettings.m_acceleration, oSettings.m_deceleration, Time.deltaTime);
	...
	void ComputeHeroBox()
	{
		if(!m_heroBoxComputed)
		{
			...
			Vector2 boxA = m_player.transform.TransformPoint(heroBox.center - 0.5f * heroBox.size);
			Vector2 boxB = m_player.transform.TransformPoint(heroBox.center + 0.5f * heroBox.size);
	...
	bool GetCatchInput()
	{
		float fYAxis = m_player.m_inputs.m_axisY.GetRawValue();
	...
	void Takecontrol()
	{
		if(!m_isControlling)
		{
			...
			m_releaseJump = string.IsNullOrEmpty(m_player.m_jump.m_button) || !Input.GetButton(m_player.m_jump.m_button);
			...
			m_player.SendMessage("APOnCharacterCatchRailings", SendMessageOptions.DontRequireReceiver);
		}
	}
	...
	void ReleaseControl(bool bJump)
	{
		if(m_isControlling)
		{
			...
			if(bJump)
			{
				float horAxis = m_player.m_inputs.m_axisX.GetRawValue();
			...
			m_player.SendMessage("APOnCharacterReleaseRailings", SendMessageOptions.DontRequireReceiver);
		}
	}
