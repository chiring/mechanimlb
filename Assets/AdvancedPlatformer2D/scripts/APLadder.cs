...
public class APLadder : MonoBehaviour 
{
	...
	public class Settings
	{
		...
		public string m_animClimbUp = "ClimbUp";
		public string m_animClimbDown = "ClimbDown";
	}
	...
	void Awake()
	{
		...
		m_axisStart = transform.TransformPoint(m_boxCollider.center - axis);
		m_axisEnd = transform.TransformPoint(m_boxCollider.center + axis);
		m_axisDir = m_axisEnd - m_axisStart;
		m_axisLength = m_axisDir.magnitude;
		m_axisDir /= m_axisLength;

		// compute box in world space
		Vector2 pointA = transform.TransformPoint(m_boxCollider.center - 0.5f * m_boxCollider.size);
		Vector2 pointB = transform.TransformPoint(m_boxCollider.center + 0.5f * m_boxCollider.size);
	...
	void LateUpdate()
	{
		...
		if(m_isControlling)
		{
			...
			// set speed
			float fSpeed = fAbsY / oSettings.m_maxSpeed;
	...
	void Takecontrol()
	{
		if(!m_isControlling)
		{
			...
			m_player.SendMessage("APOnCharacterCatchLadder", SendMessageOptions.DontRequireReceiver);
		}
	}
	...
	void ReleaseControl(bool bJump)
	{
		if(m_isControlling)
		{
			...
			m_player.SendMessage("APOnCharacterReleaseLadder", SendMessageOptions.DontRequireReceiver);
		}
	}
